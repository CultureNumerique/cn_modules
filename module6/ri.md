...
C'est cette **recherche d'information** qui fait l'objet de ce cours. En particulier, nous allons étudier l'outil numérique utilisé quotidiennement par chacun d'entre nous : le **moteur de recherche d'information**. Nous nous intéressons exclusivement aux ressources textuelles et aux moteurs de recherche du web dans le cadre de ce cours. 
...
La fonction première d'un moteur de recherche est de vous permettre de *poser une requête*  et recevoir en réponse une partie d'un ensemble de ressources (documents texte, images, vidéos,...).  L'ensemble des ressources sur lequel les requêtes sont posées est accessible au moteur de recherche et organisé de façon à répondre efficacement. C'est la phase d'*indexation*, autrefois manuelle dans les bibliothèques et aujourd'hui largement automatisée qui est responsable de cette organisation. Nous décrivons dans une première partie cette phase d'indexation. 

La réponse peut être exacte et exhaustive pour certaines requêtes simples et bien formalisées, mais le plus souvent la recherche est approximative et incomplète. Le moteur renvoie *une liste ordonnée de documents selon un score de pertinence*. Nous décrivons les méthodes de recherche approximatives utilisées et les scores de pertinence. Ces scores combinent d'une part, une mesure de la proximité entre une requête et le contenu d'une page, avec d'autre part, un **score de notoriété**.  Ce score de notoriété mesure l'importance d'une page dans le réseau constitué des pages Web reliées entre elles par des hyperliens. Il est calculé par le fameux algorithme **PageRank** qui a contribué à l'essor de `Google` à la fin du vingtième siècle. 
...
- Si la requête ne contient qu'un seul mot, on cherche par dichotomie si ce mot est un token présent dans l'index  et on renvoie la liste, associée au token dans l'index, des documents donnés par leurs identifiants.
- Si la requête est composée de deux mots, le moteur procède comme suit : 
	- il cherche par dichotomie le premier mot dans l'index et renvoie la liste des identifiants de documents 
	- il fait de même pour le deuxième mot ;  puis il construit la liste des identifiants qui apparaissent dans les deux listes : le moteur calcule donc une /intersection/ entre les deux listes. 
	- il renvoie enfin cette liste comme réponse à la requête. Ce principe peut être étendu pour des requêtes à  plusieurs mots. 

Donc, une requête à plusieurs mots est interprétée avec un opérateur `ET` car on souhaite que tous les mots soient présents. On  peut aussi écrire des requêtes avec les opérateurs logiques `OU` et `NON` qui correspondront à d'autres opérations sur les listes (union ou différence).
...
[markdown]
Nous avons dit que l'index mémorisait une liste d'identifiants de documents pour chaque token du vocabulaire. L'emploi du mot *liste*, (plutôt qu'ensemble), suggère que les éléments sont rangés dans un ordre. Mais à votre avis, est-ce que ranger les identifiants de documents par exemple dans l'ordre croissant peut avoir un effet sur la rapidité de la recherche et pourquoi ?
{####Oui, car il est plus rapide de calculer l'intersection quand les listes sont triées par ordre croissant.}
```
...
À cause de cet ordonnancement des résultats, des questions éthiques d'accès à l'information se posent, et plus évidemment encore si la liste est tronquée aux quelques documents de plus haut score, ou même du premier document. 
...
pertinents*. En fait *plus pertinent* signifie ici *plus discriminants* dans le sens où vous savez qu'il y a moins de documents contenant ces mots. Une troisième représentation, appelée **term frequency -- inverse document frequency (tf-idf)**, pénalise donc les mots fréquents et favorise les mots rares.  Dans cette représentation, on multiplie la fréquence d'apparition (*tf* pour term frequency) par un facteur (*idf* ou inverse document frequency). Nous ne donnons pas la définition mathématique de l'idf mais il suffit de retenir que l'idf d'un mot

- est *grand pour un mot rare* qui apparaît dans peu de documents
- est *petit pour un mot fréquent* qui apparaît dans beaucoup de documents
